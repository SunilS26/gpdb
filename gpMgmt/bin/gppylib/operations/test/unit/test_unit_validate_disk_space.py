...
class RelocateSegmentPairTestCase(GpTestCase):
    ...
    def test_validate_disk_space_less_than_10per(self, mock1, mock2):
        ...
        self.mock_logger.info.assert_has_calls([call(
            'Filesystem /tmp/disk1 has 1025 kB available, but target host tdw1 directory seg1, tbl1 has 1024 kB.')],
                                               self.mock_logger.info.call_args_list)
        self.assertEqual(True, result)

    @patch('gppylib.operations.validate_disk_space.ask_yesno', return_value=False)
    @patch('gppylib.operations.validate_disk_space.RelocateDiskUsage._determine_source_disk_usage', return_value=None)
    def test_validate_disk_space_less_than_10per_prompt(self, mock1, mock2):
        RelocateDiskUsage._target_host_filesystems.return_value  = MagicMock()
        fs = {}
        fs['tdw1'] = [FileSystem('/tmp/disk1', disk_free=int(1025), disk_required=int(1024), directories=('seg1', 'tbl1'))]
        fs['tdw2'] = [FileSystem('/tmp/disk2', disk_free=int(1025), disk_required=int(1024), directories=('seg2', 'tbl2'))]
        RelocateDiskUsage._target_host_filesystems.return_value = fs
        ...
        self.mock_logger.info.assert_has_calls([call('Filesystem /tmp/disk1 has 1025 kB available, but target host tdw1 directory seg1, tbl1 has 1024 kB.'),
    ...
    @patch('gppylib.operations.validate_disk_space.RelocateDiskUsage._disk_usage', side_effect=[ {'/data/seg1': 1024},
                                                                                                ...
                                                                                                {'/data/seg2': 186},
                                                                                                {'/data/tbl2' : 1024}])
    @patch('gppylib.operations.validate_disk_space.get_tablespace_locations', side_effect=[[('sdw1', 1, '/data/tbl1')], [('sdw2', 1, '/data/tbl2')]])
    ...
    #@patch('gppylib.operations.validate_disk_space.RelocateDiskUsage._target_filesystems')
    def test_target_host_filesystems(self):
        ...
        #self.assertIsInstance(res[self.pairs[1].target_hostaddr], FileSystem)
    ...
    def test_target_filesystem(self, mock1, mock2, mock3, mock4):
        ...
        #TO-Do : Check directories , it was nopt aking as set/tuple and taking char by char not string
